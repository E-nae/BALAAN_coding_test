<template>
  <div id="app">
    <form id=UserSignUp1 v-if="level" v-on:submit.prevent>
      ...
      <button id='next' @click="userValidStep1()">다 음</button>
    </form>
    ...
    <form id=UserSignUp2 v-if="NextLevel2" v-on:submit.prevent>
      ...
      <button id="before" @click="goToStep1()">이 전</button>
      <button id="next" @click="userValidStep2()">다 음</button>
    </form>

    <form id=SignEnd v-if="NextLevel3" v-on:submit.prevent>
      <div class="input_group">
      <p>카드 번호</p>
        <input type="text" class="CardNumber" maxlength="4" v-model="cardVal1" pattern="[0-9]+" id="card_no1">&nbsp;-&nbsp;
        <input type="text" class="CardNumber" maxlength="4" v-model="cardVal2" pattern="[0-9]+" id="card_no2">&nbsp;-&nbsp; 
        <input type="text" class="CardNumber" maxlength="4" v-model="cardVal3" pattern="[0-9]+" id="card_no3">&nbsp;-&nbsp; 
        <input type="text" class="CardNumber" maxlength="4" v-model="cardVal4" pattern="[0-9]+" id="card_no4">
      </div>
      <button id="next" @click="userValidStep3()">완료</button>
    </form>
...
export default {
  ...
  methods: {
    cardValid3(){
      let cardNum = this.cardVal1 + this.cardVal2 + this.cardVal3 + this.cardVal4;
      let newCardNum = Array.from(cardNum);
      
        for(let i=0; i < 8; i++){ 
          var index = 14 - (2*i); 
          var newNum = newCardNum[index] * 2; 
          if (newNum > 9) {
            newNum = parseInt(newNum/10) + newNum%10; 
          }
          newCardNum[index] = newNum;
      }
        var sum = 0;
        for(let i=0; i < newCardNum.length; i++){
          sum += parseInt(newCardNum[i]);
        }
        if(sum%10 == 0){
          return true;
        } 
    ...
    userValidStep3(){
      if(CardValid(this.cardVal) == false || this.cardVal =="" || this.cardValid3() == false){
